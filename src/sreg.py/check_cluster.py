...
def check_cluster(df):
    ...
    cov_same = df.groupby('G.id').agg({col: lambda x: x.nunique() == 1 for col in cov_names}).reset_index()
...
def check_cluster_lvl(G_id, S, D, Ng):
    dta_check_lvl = pd.DataFrame({'G.id': G_id, 'S': S, 'D': D, 'Ng': Ng})
    unique_clusters = dta_check_lvl['G.id'].unique()

    for cluster in unique_clusters:
        subset_dta = dta_check_lvl[dta_check_lvl['G.id'] == cluster]
